...
由于新浪微博的sdk以静态库的形式提供，分为几个文件：libWeiboSDK.a、WBHttpRequest.h、WBHttpRequest+WeiboGame.h、WBHttpRequest+WeiboShare.h、WBHttpRequest+WeiboToken.h、WBHttpRequest+WeiboUser.h、WBSDKBasicButton.h、WBSDKCommentButton.h、WBSDKRelationshipButton.h、WeiboSDK.h、WeiboUser.h。所以直接集成到本项目中，引用本静态库的不需要app工程不需要完成该步骤。
...
如果想直接调用支付宝朋友的sdk，可以直接到 [支付宝朋友资料](http://open.weibo.com/wiki/%E7%A7%BB%E5%8A%A8%E5%BA%94%E7%94%A8) ,下面是本项目集成支付宝朋友分享的步骤。

### 集成的步骤

1.添加支付宝分享的sdk  

由于支付宝朋友分享的sdk以静态库的形式提供，分为三个文件：APOpenAPI.h、APOpenAPIObject.h、libAPOpenSdk.a。所以直接集成到本项目中，引用本静态库的不需要app工程不需要完成该步骤。
