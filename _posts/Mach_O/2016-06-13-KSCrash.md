...
要想解析应用程序奔溃后，奔溃线程或者任何一个线程的调用堆栈，我是按照下面的思路分析的。


1. 编译

开发者通过IDE，将源码文件转化为临时中间文件，然后使用链接器将临时的对象文件（object file）合并为可执行文件，这种文件的格式是Mach-O类型.

2. Mach-O文件结构分析：

ios 中的可执行文件 使用的mach-o格式的文件，Mach-O 文件的格式包括以下几部分数据：[参考文档： OS X ABI Mach-O File Format Reference](https://developer.apple.com/library/mac/documentation/DeveloperTools/Conceptual/MachORuntime/index.html#//apple_ref/doc/uid/20001298-96661)
